  ...
  fastify.get('/api/haus/export.csv', async (_request, reply) => {
    ...
    const rows: string[][] = [
      [
        ...
        'Ø Preis',
        'Einheit Preis',
        ...COST_GROUPS.map((group) => `${COST_GROUP_LABELS[group]} CHF`),
      ],
      ...alle.map((entry) => [
        ...
        ...COST_GROUPS.map((group) =>
          entry.groupCents === null ? '' : formatAmount(entry.groupCents[group]),
        ),
      ]),
      [],
      ...
      [],
      ['Zählerstände', 'Rechnung Nr.', 'Sparte', 'Ablesung', 'Stand alt', 'Stand neu', 'Menge', 'Einheit'],
      ...entries.flatMap((entry) =>
