...
| Browser zeigt eine alte Fassung, die installierte PWA aber die neue | Im Ausgabeordner lag eine Datei auf `/` (z. B. ein `index.html` aus der Zeit vor dem Umzug nach `/app/`). Netlify zieht eine vorhandene Datei jeder Weiterleitung vor – `manager.alae.app` lieferte sie aus, statt auf `/app/` umzuleiten. Die PWA startet direkt auf `/app/` und war deshalb aktuell. Der Ausgabeordner wird seither vor jedem Build geleert, und die Weiterleitung auf `/` steht auf `force` |
| Nach einem Merge erscheint die neue Fassung nicht | In Netlify unter **Deploys** steht „Auto Publishing Locked". Netlify baut dann zwar weiter, veröffentlicht aber nur den festgehaltenen Deploy. Über **Unlock to start auto publishing** lösen |
